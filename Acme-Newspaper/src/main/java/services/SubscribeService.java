...
public class SubscribeService {
	...
	public Subscribe subscribe(final Newspaper n, final CreditCard cc, final Customer c) {

		final Subscribe sub = new Subscribe();
		sub.setCreditCard(cc);
		sub.setNewspaper(n);
		if (LoginService.isPrincipalCustomer())
			sub.setCustomer((Customer) this.loginService.getPrincipalActor());
		else if (LoginService.isPrincipalUser())
			sub.setCustomer(c);
		//		s.getCustomer().getSubscribes().add(s);
		//		s.getNewspaper().getSubscribes().add(s);
