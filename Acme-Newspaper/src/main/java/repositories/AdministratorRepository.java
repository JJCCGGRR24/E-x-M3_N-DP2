...
public interface AdministratorRepository extends JpaRepository<Administrator, Integer> {
	...
	@Query("select count(n)*1.0/(select count(t) from Newspaper t) from Newspaper n where n.deprived = 1")
	Double queryA5();
	...
	@Query("select (count(a)*1.0)/(select count(aa) from Newspaper aa where aa.advertisements.size = 0) from Newspaper a where a.advertisements.size >= 1")
	Double query2C1();

	@Query("select (count(a)*1.0)/(select count(aa) from Advertisement aa) from Advertisement a where a.tabooWord = true")
	Double query2C2();
	...
	@Query("select (count(a)*1.0)/(select count(aa) from Subscribe aa) from SubscribeVol a")
	Double query2B2();
