...
<display:table name="chirps" id="row" requestURI="${requestURI}"
	...
	<display:column sortProperty="moment">
		<div style="justify-content: space-between; display: flex;">
			...
			<spring:message code="event.format.dateWithoutHour2" var="patternvar"/>
				<fmt:formatDate value="${row.moment}" pattern="${patternvar}" />
...
<input type="button" name="back" value="${back}"
	onclick="javascript:relativeRedir('user/user/list.do')" />
</security:authorize>
<security:authorize access="!hasRole('USER')">
<input type="button" name="back" value="${back}"
